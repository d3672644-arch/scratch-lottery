...
function revealPrize() {
  ...
  fetch('https://scratch-lottery-production.up.railway.app/api/user/claim', {
    method: 'POST',
...
window.addEventListener('load', () => {
  ...
  // Загружаем баланс
  fetch(`https://scratch-lottery-production.up.railway.app/api/user/balance?userId=${userId}`)
    .then(r => r.json())
    .then(data => {
  ...
  // Генерируем приз
  fetch('https://scratch-lottery-production.up.railway.app/api/ticket/generate')
    .then(r => r.json())
